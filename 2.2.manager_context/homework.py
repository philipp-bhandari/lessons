...
class ContextManager:
    ...
    def __enter__(self):
        ...
        time_str = datetime.strftime(self.start_time, "%d.%m.%Y %H:%M:%S")
        print(time_str, '- время начала работы скрипта.')
        print('\n*\n*\n*\n')

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.end_time = datetime.now()
        end_time_str = datetime.strftime(self.end_time, "%d.%m.%Y %H:%M:%S")
        print('\n*\n*\n*\n')
        print(end_time_str, '- время окончания работы скрипта.')
        all_time = self.end_time - self.start_time
        all_time = all_time.total_seconds()
        print('\n*\n*\n*\n')
        print(all_time, 'секунд понадобилось, чтобы выполнить этот скрипт.')


manager = ContextManager()

with manager as test:
    def sep():
        print(30 * '-')
    ...
    i.ask_heaviest_animal(human, gray_goose, white_goose, cow, goat_horns, goat_hooves, sheep_barry, sheep_curvy,
                          chicken_koko, chicken_kuku, krya_duck)
